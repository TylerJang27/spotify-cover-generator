...
import { Playlist, PlaylistedTrack, SpotifyApi } from "@spotify/web-api-ts-sdk";
...
const findPlaylist = async (userId: string, playlistName: string) => {
  const sdk = getSdk();
  let total = 0;
...
const listTracks = async (playlist: Playlist): Promise<PlaylistedTrack[]> => {
  const sdk = getSdk();
  const tracks = [];
  let total = 0;
  ...
  if (total > tracks.length) {
    console.log(`Truncated playlist size from ${total} to ${tracks.length}`);
  }
  return tracks;
};
...
const main = () => {
  ...
  program
    ...
    // TODO(Tyler): Make playlist names optional
